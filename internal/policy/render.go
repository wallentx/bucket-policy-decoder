...
func (r renderer) describePrincipal(stmt Statement) string {
	...
	case stmt.NotPrincipal.Any:
		return "everyone except the listed principals"
	case len(stmt.NotPrincipal.Values) > 0:
		return "everyone except " + r.describePrincipalValue(stmt.NotPrincipal)
...
func (r renderer) describeCondition(operator, key string, values []string) string {
	...
	case "StringEquals", "ArnEquals", "NumericEquals", "DateEquals":
		return fmt.Sprintf("%s equals %s", key, valueText)
	case "StringNotEquals", "ArnNotEquals", "NumericNotEquals", "DateNotEquals":
		return fmt.Sprintf("%s does not equal %s", key, valueText)
	case "StringLike", "ArnLike":
		return fmt.Sprintf("%s matches %s", key, valueText)
	case "StringNotLike", "ArnNotLike":
		return fmt.Sprintf("%s does not match %s", key, valueText)
	case "NumericLessThan", "DateLessThan":
		return fmt.Sprintf("%s is less than %s", key, valueText)
	case "NumericLessThanEquals", "DateLessThanEquals":
		return fmt.Sprintf("%s is less than or equal to %s", key, valueText)
	case "NumericGreaterThan", "DateGreaterThan":
		return fmt.Sprintf("%s is greater than %s", key, valueText)
	case "NumericGreaterThanEquals", "DateGreaterThanEquals":
		return fmt.Sprintf("%s is greater than or equal to %s", key, valueText)
	case "Null":
		return fmt.Sprintf("%s is null = %s", key, valueText)
	}
	return fmt.Sprintf("%s %s %s", key, operator, valueText)
}
...
func (r renderer) describeConditionPhrase(operator, key string, values []string) (string, bool) {
	...
	valueText := r.joinQuotedValues(values)
	switch {
	case operator == "Bool" && key == "aws:SecureTransport" && len(values) == 1 && values[0] == "false":
		return "the request is not using HTTPS", true
	case operator == "Bool" && key == "aws:SecureTransport" && len(values) == 1 && values[0] == "true":
		return "the request is using HTTPS", true
	case operator == "Bool" && key == "aws:MultiFactorAuthPresent" && len(values) == 1 && values[0] == "true":
		return "MFA is present", true
	case operator == "Bool" && key == "aws:MultiFactorAuthPresent" && len(values) == 1 && values[0] == "false":
		return "MFA is not present", true
	case operator == "StringEquals" && key == "s3:x-amz-acl":
		return "the upload sets the ACL to " + valueText, true
	case operator == "StringEquals" && key == "s3:x-amz-storage-class":
		return "the upload uses storage class " + valueText, true
	case operator == "StringNotEquals" && key == "s3:x-amz-storage-class":
		return "the upload does not use storage class " + valueText, true
	case operator == "StringEquals" && key == "s3:x-amz-server-side-encryption-aws-kms-key-id":
		return "the upload is encrypted with KMS key " + valueText, true
	case operator == "StringNotEquals" && key == "s3:x-amz-server-side-encryption-aws-kms-key-id":
		return "the upload is not encrypted with KMS key " + valueText, true
